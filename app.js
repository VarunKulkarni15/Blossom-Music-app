...
lucide.createIcons();
        ...
        async function searchSaavnAPI(query) {
            try {
                // Point to our new reliable backend proxy
                const res = await fetch(`/api/saavn-search?query=${encodeURIComponent(query)}`);
                if (!res.ok) throw new Error('Saavn proxy error');
                
                const data = await res.json();
                
                // data is already formatted by the server proxy
                ...
                console.error('Saavn proxy fetch failed:', err);
                return [];
            }
        }
        ...
        function renderList() {
            ...
            vs.forEach((s, idx) => {
                ...
                // Add right-click listener safely
                row.oncontextmenu = function(e) { e.preventDefault(); openMenu(e, s.id); return false; };
